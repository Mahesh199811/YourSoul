    ...
    public partial class DiscoverViewModel : BaseViewModel
    {
        ...
        public async Task LoadPotentialMatchesAsync()
        {
            ...
            try
            {
                ...
                // Use Task.Run to move the heavy database operation to a background thread
                var matches = await Task.Run(() => _databaseService.GetPotentialMatchesAsync(currentUser));

                // Use Clear and AddRange for better performance
                PotentialMatches.Clear();

                // Create a new collection and update the observable collection at once
                if (matches.Any())
                {
                    ...
                    NoMoreMatches = false;
                    CurrentPotentialMatch = PotentialMatches[0]; // More efficient than FirstOrDefault
